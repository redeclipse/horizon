...
namespace game
{
    ...
    {
        gameent *d = getclient(cn);
        return d && d->state!=CS_SPECTATOR ? getplayercolor(d) : 0xFFFFFF;
    }
    ...
    void parsestate(gameent *d, ucharbuf &p, bool resume = false)
    {
        ...
        {
            int weap = getint(p);
            d->weapselect = clamp(weap, 0, NUMWEAPS-1);
    ...
    void parsemessages(int cn, gameent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case N_SPAWNSTATE:
            {
                ...
                parsestate(s, p);
                s->state = CS_ALIVE;
                findplayerspawn(s, -1, s->aitype);
                if(s == player1) lasthit = 0;
                ai::spawned(s);
                checkfollow();
                addmsg(N_SPAWN, "rcii", s, s->lifesequence, s->weapselect);
                break;
            }
            ...
            case N_SHOTFX:
            {
                ...
                s->weapselect = weap;
                s->ammo[weap] -= attacks[atk].use;
                s->weapwait = attacks[atk].attackdelay;
            ...
            case N_INITAI:
            {
                int bn = getint(p), on = getint(p), at = getint(p), sk = clamp(getint(p), 1, 101), pm = getint(p), col = getint(p);
                string name;
                getstring(text, p);
                filtertext(name, text, false, false, MAXNAMELEN);
                gameent *b = newclient(bn);
                if(!b) break;
                ai::init(b, at, on, sk, bn, pm, col, name);
                break;
            }
