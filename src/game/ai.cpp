...
namespace ai
{
    ...
    avoidset obstacles;
    int updatemillis = 0, iteration = 0, itermillis = 0, forceweap = -1;
    vec aitarget(0, 0, 0);

    VAR(aidebug, 0, 0, 6);
    VAR(aiforceweap, -1, -1, NUMWEAPS-1);
    ...
    bool targetable(gameent *d, gameent *e)
    {
        ...
        return e->state == CS_ALIVE && e->aitype == AI_NONE;
    }
    ...
    bool canshoot(gameent *d, int atk, gameent *e)
    {
        if(attackrange(d, atk, e->o.squaredist(d->o)) && targetable(d, e))
            return d->ammo[attacks[atk].weap] > 0 && lastmillis - d->lastaction >= d->weapwait;
        return false;
    }

    bool canshoot(gameent *d, int atk)
    {
        return !d->ai->becareful && d->ammo[attacks[atk].weap] > 0 && lastmillis - d->lastaction >= d->weapwait;
    }
    ...
    void init(gameent *d, int at, int ocn, int sk, int bn, int pm, int col, const char *name)
    {
        loadwaypoints();
    ...
    void update()
    {
        ...
        else // fixed rate logic done out-of-sequence at 1 frame per second for each ai
        {
            ...
            {
                avoid();
                forceweap = multiplayer(false) ? -1 : aiforceweap;
    ...
    bool checkothers(vector<int> &targets, gameent *d, int state, int targtype, int target, int *members)
    { // checks the states of other ai for a match
        targets.setsize(0);
        loopv(players)
        {
            gameent *e = players[i];
            if(targets.find(e->clientnum) >= 0 || e->aitype == AI_NONE) continue;
            if(members) (*members)++;
    ...
    bool enemy(gameent *d, aistate &b, const vec &pos, float guard = SIGHTMIN, int pursue = 0)
    {
        ...
        int atk = weaps[d->weapselect].attacks[ACT_SHOOT];
        loopv(players)
        {
            gameent *e = players[i];
    ...
    int isgoodammo(int weap) { return weap == WEAP_PULSE || weap == WEAP_RAIL; }

    bool hasgoodammo(gameent *d)
    {
        static const int goodweaps[] = { WEAP_PULSE, WEAP_RAIL };
        loopi(sizeof(goodweaps)/sizeof(goodweaps[0])) if(d->hasammo(goodweaps[0])) return true;
        return false;
    }

    void assist(gameent *d, aistate &b, vector<interest> &interests, bool force)
    {
        loopv(players)
        {
            gameent *e = players[i];
            if(e == d || e->aitype != AI_NONE) continue;
    ...
    bool parseinterests(gameent *d, aistate &b, vector<interest> &interests, bool override, bool ignore)
    {
        while(!interests.empty())
        {
            ...
            if(!ignore) switch(n.state)
            {
                ...
                {
                    int members = 0;
                    proceed = !checkothers(targets, d, n.state, n.targtype, n.target, &members) && members > 1;
                    break;
                }
                default: break;
            }
    ...
    void damaged(gameent *d, gameent *e)
    {
        ...
        if(checkothers(targets, d, AI_S_DEFEND, AI_T_PLAYER, d->clientnum))
        {
            loopv(targets)
            {
    ...
    void setup(gameent *d)
    {
        ...
        if(forceweap >= 0 && forceweap < NUMWEAPS) d->ai->weappref = forceweap;
        else d->ai->weappref = rnd(NUMWEAPS);
        vec dp = d->headpos();
    ...
    bool hasrange(gameent *d, gameent *e, int weap)
    {
        ...
        if(targetable(d, e))
        {
            int atk = weaps[weap].attacks[ACT_SHOOT];
            vec ep = getaimpos(d, atk, e);
    ...
    {
        gameent *e = getclient(d->ai->enemy);
        if(!d->hasammo(d->weapselect) || !hasrange(d, e, d->weapselect) || (d->weapselect != d->ai->weappref && (!isgoodammo(d->weapselect) || d->hasammo(d->ai->weappref))))
        {
            static const int weapprefs[] = { WEAP_PULSE, WEAP_RAIL };
            int weap = -1;
            if(d->hasammo(d->ai->weappref) && hasrange(d, e, d->ai->weappref)) weap = d->ai->weappref;
            else
            {
                loopi(sizeof(weapprefs)/sizeof(weapprefs[0])) if(d->hasammo(weapprefs[i]) && hasrange(d, e, weapprefs[i]))
                {
                    weap = weapprefs[i];
                    break;
                }
            }
            if(weap >= 0 && weap != d->weapselect) weapselect(weap, d);
        }
        ...
        if(aidebug > 1)
        {
            ...
            loopv(players) if(players[i]->state == CS_ALIVE && players[i]->ai)
            {
                ...
                if(aidebug >= 3)
                {
                    ...
                    {
                        particle_textcopy(pos, weaps[d->ai->weappref].name, PART_TEXT, 1);
                        pos.z += 2;
                    }
                    gameent *e = getclient(d->ai->enemy);
                    if(e)
