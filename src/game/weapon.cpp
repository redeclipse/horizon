...
namespace game
{
    ...
    ICOMMAND(getweapon, "", (), intret(player1->weapselect));

    void weapselect(int weap, gameent *d)
    {
        if(weap!=d->weapselect)
        {
            addmsg(N_WEAPSELECT, "rci", d, weap);
    ...
    void nextweapon(int dir, bool force = false)
    {
        ...
        int weap = player1->weapselect;
        loopi(NUMWEAPS)
        {
            weap = (weap + dir)%NUMWEAPS;
            if(force || player1->ammo[weap]) break;
        }
        if(weap != player1->weapselect) weapselect(weap, player1);
    ...
    int getweapon(const char *name)
    {
        ...
        {
            int len = strlen(name);
            loopi(sizeof(weaps)/sizeof(weaps[0])) if(!strncasecmp(weaps[i].name, name, len)) return i;
        }
        return -1;
    }
    ...
    {
        int weap = getweapon(name);
        if(player1->state!=CS_ALIVE || !validweap(weap)) return;
        if(force || player1->ammo[weap]) weapselect(weap, player1);
        else playsound(S_NOAMMO);
    }
    ICOMMAND(setweapon, "si", (char *name, int *force), setweapon(name, *force!=0));

    void cycleweapon(int numweaps, int *weaps, bool force = false)
    {
        if(numweaps<=0 || player1->state!=CS_ALIVE) return;
        int offset = 0;
        loopi(numweaps) if(weaps[i] == player1->weapselect) { offset = i+1; break; }
        loopi(numweaps)
        {
            int weap = weaps[(i+offset)%numweaps];
            if(weap>=0 && weap<NUMWEAPS && (force || player1->ammo[weap]))
            {
                weapselect(weap, player1);
                return;
            }
        }
        playsound(S_NOAMMO);
    ...
    {
         int numweaps = min(numargs, 3);
         int weaps[3];
         loopi(numweaps) weaps[i] = getweapon(args[i].getstr());
         cycleweapon(numweaps, weaps);
    });
    ...
    void weaponswitch(gameent *d)
    {
        ...
        int s = d->weapselect;
        if(s!=WEAP_PULSE && d->ammo[WEAP_PULSE])     s = WEAP_PULSE;
        else if(s!=WEAP_RAIL && d->ammo[WEAP_RAIL])  s = WEAP_RAIL;
        weapselect(s, d);
    }
    ...
    ICOMMAND(weapon, "V", (tagval *args, int numargs),
    {
        ...
        loopi(3)
        {
            ...
            {
                int weap = getweapon(name);
                if(validweap(weap) && weap != player1->weapselect && player1->ammo[weap]) { weapselect(weap, player1); return; }
    ...
    void newprojectile(const vec &from, const vec &to, float speed, bool local, int id, gameent *owner, int atk)
    {
        ...
        p.from = from;
        p.to = to;
        p.offset = hudweaporigin(attacks[atk].weap, from, to, owner);
        p.offset.sub(from);
        p.speed = speed;
        p.local = local;
        p.owner = owner;
    ...
    void shoteffects(int atk, const vec &from, const vec &to, gameent *d, bool local, int id, int prevaction)     // create visual effect from a shot
    {
        ...
        switch(atk)
        {
            ...
            case ATK_RAIL_SHOOT:
                ...
                particle_flare(hudweaporigin(weap, from, to, d), to, 500, PART_RAIL_TRAIL, 0x50CFE5, 0.5f);
                if(d->muzzle.x >= 0)
                    particle_flare(d->muzzle, d->muzzle, 140, PART_RAIL_MUZZLE_FLASH, 0x50CFE5, 2.75f, d);
                adddynlight(hudweaporigin(weap, d->o, to, d), 35, vec(0.25f, 0.75f, 1.0f), 75, 75, DL_FLASH, 0, vec(0, 0, 0), d);
    ...
    void particletrack(physent *owner, vec &o, vec &d)
    {
        ...
        if(pl->muzzle.x < 0 || pl->lastattack < 0 || attacks[pl->lastattack].weap != pl->weapselect) return;
        float dist = o.dist(d);
        o = pl->muzzle;
        if(dist <= 0) d = o;
        else
    ...
    void shoot(gameent *d, const vec &targ)
    {
        ...
        if(attacktime<d->weapwait) return;
        d->weapwait = 0;
        if(!d->attacking) return;
        int weap = d->weapselect, act = d->attacking, atk = weaps[weap].attacks[act];
        ...
        d->weapwait = attacks[atk].attackdelay;
        if(attacks[atk].action != ACT_MELEE && d->ai) d->weapwait += int(d->weapwait*(((101-d->skill)+rnd(111-d->skill))/100.f));
