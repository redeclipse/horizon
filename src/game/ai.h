...
namespace ai
{
    ...
    extern void init(gameent *d, int at, int on, int sk, int bn, int pm, int col, const char *name);
    extern void update();
    extern void avoid();
    extern void think(gameent *d, bool run);

    extern bool badhealth(gameent *d);
    extern bool checkothers(vector<int> &targets, gameent *d = NULL, int state = -1, int targtype = -1, int target = -1, int *members = NULL);
    ...
    extern void assist(gameent *d, aistate &b, vector<interest> &interests, bool force = false);
