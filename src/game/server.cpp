...
namespace game
{
    ...
    const char *gameident() { return "Red Eclipse Horizon"; }
}
...
namespace server
{
    ...
    struct servstate : gamestate
    {
        ...
        {
            return gamemillis - lastshot >= weapwait;
        }
    ...
    {
        if(!sents.inrange(i) || !sents[i].spawned) return false;
    ...
    void setupdemoplayback()
    {
        ...
        {
            lilswap(&hdr.version, 2);
            if(hdr.version!=DEMO_VERSION) formatstring(msg, "demo \"%s\" requires an %s version of Red Eclipse Horizon", file, hdr.version<DEMO_VERSION ? "older" : "newer");
            else if(hdr.protocol!=PROTOCOL_VERSION) formatstring(msg, "demo \"%s\" requires an %s version of Red Eclipse Horizon", file, hdr.protocol<PROTOCOL_VERSION ? "older" : "newer");
        }
        if(msg[0])
        {
            DELETEP(demoplayback);
    ...
    void sendspawn(clientinfo *ci)
    {
        ...
        sendf(ci->ownernum, 1, "rii5v", N_SPAWNSTATE, ci->clientnum, gs.lifesequence,
            ...
            gs.weapselect, NUMWEAPS, gs.ammo);
    ...
    void sendresume(clientinfo *ci)
    {
        ...
        sendf(-1, 1, "ri3i7vi", N_RESUME, ci->clientnum, gs.state,
            ...
            gs.lifesequence,
            gs.health, gs.maxhealth,
            gs.weapselect, NUMWEAPS, gs.ammo, -1);
    }
    ...
    void dodamage(clientinfo *target, clientinfo *actor, int damage, int atk, const vec &hitpush = vec(0, 0, 0))
    {
        ...
        ts.dodamage(damage);
        if(target!=actor) actor->state.damage += damage;
        ...
        {
            target->state.deaths++;
            int fragvalue = target==actor ? -1 : 1;
            actor->state.frags += fragvalue;
            if(fragvalue>0) actor->state.effectiveness += fragvalue*1/float(max(clients.length()-1, 1));
    ...
    void shotevent::process(clientinfo *ci)
    {
        ...
        if(gs.ammo[weap]<=0 || (attacks[atk].range && from.dist(to) > attacks[atk].range + 1))
            return;
        gs.ammo[weap] -= attacks[atk].use;
        gs.lastshot = millis;
        gs.weapwait = attacks[atk].attackdelay;
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            ...
            case N_WEAPSELECT:
            {
                ...
                if(!cq || cq->state.state!=CS_ALIVE || !validweap(weapselect)) break;
                QUEUE_AI;
                QUEUE_MSG;
                break;
            }
            ...
            case N_SPAWN:
            {
                int ls = getint(p), weapselect = getint(p);
                if(!cq || (cq->state.state!=CS_ALIVE && cq->state.state!=CS_DEAD && cq->state.state!=CS_EDITING) || ls!=cq->state.lifesequence || cq->state.lastspawn<0 || !validweap(weapselect)) break;
                cq->state.lastspawn = -1;
                cq->state.state = CS_ALIVE;
                cq->state.weapselect = weapselect;
                cq->exceeded = 0;
                QUEUE_AI;
                QUEUE_BUF({
