...
namespace game
{
    ...
    void otherplayers(int curtime)
    {
        loopv(players)
        {
            ...
            {
                if(lastmillis - d->lastaction >= d->weapwait) d->weapwait = 0;
            }
    ...
    void killed(gameent *d, gameent *actor)
    {
        ...
        if(d==h || actor==h)
        {
            ...
            if(d==actor)
                conoutf(CON_FRAG_SELF, "\f2%s suicided%s", dname, d==player1 ? "!" : "");
            else
            {
                if(d==player1) conoutf(CON_FRAG_SELF, "\f2%s got fragged by %s", dname, aname);
                else conoutf(CON_FRAG_SELF, "\f2%s fragged %s", aname, dname);
            }
        }
        deathstate(d);
        ai::killed(d, actor);
    ...
    void startmap(const char *name)   // called just after a map load
    {
        ...
        else findplayerspawn(player1, -1, player1->aitype);
    ...
    const char *defaultcrosshair(int index)
    {
        switch(index)
        {
            case 1: return "interface/crosshair/default_hit.png";
            default: return "interface/crosshair/default.png";
        }
    }
