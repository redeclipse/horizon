...
struct md3 : vertmodel, vertloader<md3>
{
    ...
    bool loaddefaultparts()
    {
        ...
        part &mdl = addpart();
        defformatstring(name1, "model/%s/tris.md3", name);
        mdl.meshes = sharemeshes(path(name1));
        if(!mdl.meshes)
        {
            defformatstring(name2, "model/%s/tris.md3", pname);    // try md3 in parent folder (vert sharing)
    ...
    bool load()
    {
        formatstring(dir, "model/%s", name);
        defformatstring(cfgname, "model/%s/md3.cfg", name);
