...
namespace UI
{
    ...
    {
        int model, color, weapon;

        void setup(int model_, int color_, int weapon_, float minw_, float minh_)
        {
            Preview::setup(minw_, minh_);
            model = model_;
            color = color_;
    ...
    ICOMMAND(uiplayerpreview, "iiiffe", (int *model, int *color, int *weapon, float *minw, float *minh, uint *children),
        BUILD(PlayerPreview, o, o->setup(*model, *color, *weapon, *minw, *minh), children));
