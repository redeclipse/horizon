...
bool soundsample::load(const char *dir, bool msg)
{
    ...
    {
        formatstring(filename, "sound/%s%s%s", dir, name, exts[i]);
    ...
    conoutf(CON_ERROR, "failed to load sample: sound/%s%s", dir, name);
    return false;
}
...
void initmumble()
{
    ...
    if(VALID_MUMBLELINK) return;
        ...
        if(mumblelink)
        {
            ...
            if(mumbleinfo) wcsncpy(mumbleinfo->name, L"Horizon", 256);
        }
    #elif defined(_POSIX_SHARED_MEMORY_OBJECTS)
        ...
        if(mumblelink >= 0)
        {
            ...
            if(mumbleinfo != (MumbleInfo *)-1) wcsncpy(mumbleinfo->name, L"Red Eclipse Horizon", 256);
        }
    #endif
