...
void loadcaustics(bool force)
{
    ...
    loopi(NUMCAUSTICS)
    {
        defformatstring(name, "<grey><noswizzle>media/texture/caustic/caust%.2d.png", i);
...
{
    if(!vertwater) renderflatwater(m.o.x, m.o.y, m.o.z, m.rsize, m.csize, mat);
...
void renderlava()
{
    loopk(4)
    {
        if(lavasurfs[k].empty() && lavafallsurfs[k].empty()) continue;
