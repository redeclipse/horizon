...
VARF(rhnearplane, 1, 16, 16, clearradiancehintscache());
...
static Shader *deferredlightshader = NULL, *deferredmsaapixelshader = NULL, *deferredmsaasampleshader = NULL;
...
Shader *loaddeferredlightshader(const char *type = NULL)
{
    ...
    bool multisample = false, avatar = true;
    if(type)
    {
...
{
    Shader *s = msaapass <= 0 ? deferredlightshader : (msaapass > 1 ? deferredmsaasampleshader : deferredmsaapixelshader);
