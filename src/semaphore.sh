...
SEMABUILD_TARGET='qreeves@icculus.org:/webspace/redeclipse.net/horizon'
SEMABUILD_APT='DEBIAN_FRONTEND=noninteractive apt-get'
SEMABUILD_SOURCE="http://redeclipse.net/horizon"
#SEMABUILD_MODULES=`cat "${SEMABUILD_PWD}/.gitmodules" | grep '\[submodule "[^.]' | sed -e 's/^.submodule..//;s/..$//' | tr "\n" " " | sed -e 's/ $//'`
#SEMABUILD_ALLMODS="horizon ${SEMABUILD_MODULES}"
SEMABUILD_MODULES=""
SEMABUILD_ALLMODS="horizon"
